...
pub fn review_repo(name: &str, dirs: &ProjectDirs) {
	env::set_current_dir(dirs.cache_dir().join(name).join(PREFETCH_DIR)).unwrap_or_else(|_| panic!("Faild to cd into build dir for {}", name));
	loop {
	...
	env::set_current_dir("..")
		.unwrap_or_else(|_| panic!("{}:{} Failed to move to parent repo after review", file!(), line!()));
	fs::rename(PREFETCH_DIR, "build")
		.unwrap_or_else(|_| panic!("Failed to move temporary directory '{}' to 'build'", PREFETCH_DIR));
}
