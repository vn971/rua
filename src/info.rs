use crate::config::Config;
use crate::fmt::{date, opt, print_indent};

use failure::Error;
use std::collections::HashMap;
use term_size::dimensions_stdout;

pub fn info(conf: &Config, pkgs: &[String], verbose: bool) -> Result<(), Error> {
    let mut hm = HashMap::with_capacity(pkgs.len());

    for chunk in pkgs.chunks(150) {
        let pkgs = raur::info(&chunk)?;
        for pkg in pkgs {
            hm.insert(pkg.name.clone(), pkg);
        }
    }

    let mut info = Vec::with_capacity(hm.len());

    for pkg in pkgs {
        if let Some(pkg) = hm.get(pkg) {
            info.push(pkg)
        } else {
            println!(
                "{} package '{}' was not found",
                conf.color.error.paint("error:"),
                pkg
            );
        }
    }

    let cols = dimensions_stdout().map(|x| x.0);
    let print = |k: &str, v: &str| print(conf, 18, cols, k, v);
    let print_list = |k: &str, v: &[_]| print_list(conf, 18, cols, k, v);

    for pkg in info {
        print("Repository", "aur");
        print("Name", &pkg.name);
        print("Description", opt(&pkg.description));
        print("URL", opt(&pkg.url));
        print("AUR URL", &pkg.package_base);
        print_list("Groups", &pkg.groups);
        print_list("Licenses", &pkg.license);
        print_list("Provides", &pkg.provides);
        print_list("Depends On", &pkg.depends);
        print_list("Make Deps", &pkg.make_depends);
        print_list("Check Deps", &pkg.check_depends);
        print_list("Optional Deps", &pkg.opt_depends);
        print_list("Conflicts With", &pkg.conflicts);
        print("Maintainer", opt(&pkg.maintainer));
        print("Votes", &pkg.num_votes.to_string());
        print("Popularity", &pkg.popularity.to_string());
        print("First Submitted", &date(pkg.first_submitted));
        print("Last Modified", &date(pkg.last_modified));
        print(
            "Out Of Date",
            pkg.out_of_date
                .map(date)
                .as_ref()
                .map(String::as_ref)
                .unwrap_or("No"),
        );

        if verbose {
            print("ID", &pkg.id.to_string());
            print("Package Base ID", &pkg.package_base_id.to_string());
            print_list("Keywords", &pkg.keywords);
            print("Snapshot URL", &pkg.url_path);
        }

        println!();
    }

    Ok(())
}

fn print(conf: &Config, indent: usize, cols: Option<usize>, k: &str, v: &str) {
    print_indent(conf, false, indent, cols, k, v.split_whitespace())
}

fn print_list(conf: &Config, indent: usize, cols: Option<usize>, k: &str, v: &[String]) {
    if v.is_empty() {
        print(conf, indent, cols, k, "None")
    } else {
        print_indent(conf, true, indent, cols, k, v.iter().map(|s| s.as_str()))
    }
}
