...
impl FlatSrcinfo {
	pub fn new(path: PathBuf) -> FlatSrcinfo {
		...
		let file = File::open(&path).unwrap_or_else(|_| panic!("Cannot open SRCINFO at path {:?}", path));
		...
		for line in file.lines() {
			...
			let key = split.get(0).unwrap_or_else(|| panic!("Unexpected line {} in .SRCINFO", line)).to_string();
